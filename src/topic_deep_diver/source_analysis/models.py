...
class BiasAnalysis:
    ...
    political_bias: Optional[str] = None  # e.g., "left", "right", "center"
...
class SourceAnalysisResult:
    ...
    credibility: CredibilityScore = field(default_factory=lambda: CredibilityScore(0.5, 0.5, 0.5, 0.5))
    bias: BiasAnalysis = field(default_factory=lambda: BiasAnalysis(BiasType.NONE, 0.0))
    deduplication: DeduplicationResult = field(default_factory=lambda: DeduplicationResult(False, 0.0))
    ...
    def should_include(self) -> bool:
        ...
        return (
            self.credibility.overall_score >= 0.6 and
            self.bias.bias_score <= 0.7 and
            not self.deduplication.is_duplicate
        )
...
class AnalysisConfig:
    ...
    deduplication_settings: Dict[str, Any] = field(default_factory=lambda: {
    ...
    domain_authority_scores: Dict[str, float] = field(default_factory=lambda: {
        # Academic domains
        ".edu": 0.95,
        ".ac.uk": 0.95,
        ".ac.au": 0.95,
