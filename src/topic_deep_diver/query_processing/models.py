...
class SubQuestion(BaseModel):
    ...
    dependencies: list[str] = Field(default_factory=list, description="IDs of dependent questions")
...
class QueryAnalysis(BaseModel):
    ...
    complexity_score: float = Field(..., ge=0.0, le=1.0, description="Topic complexity (0-1)")
...
class QueryPlan(BaseModel):
    ...
    total_estimated_time: int = Field(..., ge=1, description="Total estimated time in seconds")
    total_estimated_sources: int = Field(..., ge=1, description="Total estimated sources")
    priority_order: list[str] = Field(..., description="Execution order of sub-questions")
...
class TaxonomyNode(BaseModel):
    ...
    relevance_score: float = Field(..., ge=0.0, le=1.0, description="Relevance to topic")
